...
options obs=10000;
ods tagsets.slider file='D:\test2.html' style=slider options(doc='help');
...
/*
proc report data=erreurs nowd; 
column bloq lib_err count percent count2;
define bloq / order ;
define lib_err / display "Libéllé de l'erreur" style(column)=[cellwidth=20cm];
define percent / format=percent7.3 style(column)=slider[tagattr="slider-80"] style=[just=c] ;
define count /  style=[tagattr="format:### ### ### ###"];
define count2 / style=[tagattr="format:### ### ### ###"];
break after bloq / summarize skip ol style=[font_weight=bold just=c background=CXFFFCD8];
rbreak after / summarize skip dol style=[font_weight=bold just=c background=CXA37AE5 foreground=CXF0FFF0];

compute bloq;
if bloq ne '' then call define (_col_,'style','style=[font_weight=bold]');
endcomp;
run;
*/
